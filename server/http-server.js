// Serigraph HTTP worker — launched and restarted by main.js.
...
import { ROOT, LIBRARY_ROOT, ENV_PATH, LIBRARY_LOCKED, PREFERENCES_FILE } from './env.js';
...
const updater = createUpdater({
  ...
  protectedPaths: [MAPS_DIR, PROJECTS_DIR, TRASH_DIR, ENV_PATH, PREFERENCES_FILE],
});
...
function localAction(req, header, token) {
  return ['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.socket.remoteAddress)
    && LOCAL_HOSTS.test(req.headers.host ?? '') && req.headers.origin === `http://${req.headers.host}`
    && (!req.headers['sec-fetch-site'] || req.headers['sec-fetch-site'] === 'same-origin')
    && req.headers[header] === token && req.headers['x-serigraph-library'] === LIBRARY_ID;
}
...
async function handleLibrary(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/api/library') return json(res, 200, {
    ...
    message: librarySettingsEnabled ? '' : LIBRARY_LOCKED
      ...
      : 'Folder changes are available only in the local app, not exports or managed/shared installations.',
  });
  if (!librarySettingsEnabled || !localAction(req, 'x-serigraph-settings-token', libraryToken)) return json(res, 403, { error: 'Folder changes must be requested from this local Serigraph app.' });
  if (req.method !== 'POST' || !['/api/library/preview', '/api/library/switch'].includes(url.pathname) || url.search) return json(res, 405, { error: 'Unknown folder action.' });
  let body;
  try { body = JSON.parse(await readBody(req, 8192)); } catch { return json(res, 400, { error: 'Invalid folder request.' }); }
  if (checkingUpdate || applyingUpdate || switchingLibrary) return json(res, 409, { error: 'Wait for the current maintenance operation to finish.' });
  if (url.pathname.endsWith('/preview')) {
    try {
      const preview = await inspectLibraryDirectory(body?.path);
      libraryPlan = { ...preview, checkedAt: Date.now(), nonce: randomUUID() };
      return json(res, 200, libraryPlan);
    } catch (error) { libraryPlan = null; return json(res, 400, { error: error.message }); }
  }
  if (!libraryPlan || body?.nonce !== libraryPlan.nonce || body?.trusted !== true || Date.now() - libraryPlan.checkedAt > 600_000) return json(res, 409, { error: 'Preview and approve the folder again before switching.' });
  if (activeOperations || hasRunningAgents() || sseClients.size > 1) return json(res, 409, { error: 'Finish saves, sync, and AI/agent work, and close other Serigraph tabs before switching folders.' });
  const approved = libraryPlan;
  switchingLibrary = true;
  try {
    const checked = await inspectLibraryDirectory(approved.path);
    if (checked.identity !== approved.identity || checked.path !== approved.path || checked.hasEnv !== approved.hasEnv) throw new Error('The folder changed. Preview it again before switching.');
    await saveLibraryPreference(PREFERENCES_FILE, checked.path);
    res.once('finish', () => process.send({ type: 'restart' }, error => {
      if (error) console.error('[serigraph] Folder preference saved; restart the server manually.');
      shutdown('approved project-files folder change');
    }));
    return json(res, 200, { status: 'restarting' });
  } catch (error) {
    switchingLibrary = false;
    libraryPlan = null;
    return json(res, 400, { error: error.message.startsWith('The folder changed') ? error.message : 'Could not save the folder preference. Check folder permissions, then preview and try again.' });
  }
}
...
async function handleUpdates(req, res, url) {
  if (url.pathname === '/api/updates' && req.method === 'GET') return json(res, 200, updateStatus());
  // Stronger than ordinary library writes: no LAN mode, proxy, cross-origin
  // browser, stale library, or request-supplied command/ref/URL can update code.
  ...
  if (url.pathname.endsWith('/check')) {
    // One fetch per minute at most across every tab; passive checks use an
    // hourly cadence. Failures are cached too, so offline installs back off.
    if (Date.now() - updateState.checkedAt < 60_000) return json(res, 200, updateStatus());
    checkingUpdate = true;
    try {
      updatePlan = await updater.check();
      if (updatePlan.current !== runningRevision) throw new Error('The installed code changed outside the app. Restart Serigraph before updating.');
      updateState = updatePlan;
    } catch (error) {
      updatePlan = null;
      updateState = { status: 'blocked', current: runningRevision, checkedAt: Date.now(), message: error.message };
    } finally { checkingUpdate = false; }
    return json(res, 200, updateStatus());
  }
  let body;
  try { body = JSON.parse(await readBody(req, 2048)); } catch { return json(res, 400, { error: 'Invalid update request.' }); }
  // Recheck after the asynchronous body read; concurrent applies must not
  // both pass the initial gate.
  if (applyingUpdate || checkingUpdate || switchingLibrary) return json(res, 409, { error: 'A maintenance operation is already in progress.' });
  if (!updatePlan || updatePlan.status !== 'available' || body?.target !== updatePlan.target || body?.current !== updatePlan.current) {
    return json(res, 409, { error: 'Check for updates again before installing.' });
  }
  if (activeOperations || hasRunningAgents() || sseClients.size > 1) {
    return json(res, 409, { error: 'Finish saves, sync, and AI/agent work, and close other Serigraph tabs before updating.' });
  }
  applyingUpdate = true;
  try {
    updateState = await updater.apply(updatePlan);
    res.once('finish', () => process.send({ type: 'restart' }, error => {
      if (error) { applyingUpdate = false; return; }
      shutdown('approved update');
    }));
    return json(res, 200, { ...updateStatus(), status: 'restarting', target: updatePlan.target });
  } catch (error) {
...
const server = createServer(async (req, res) => {
  ...
  try {
    const updateRequest = url.pathname === '/api/updates' || url.pathname.startsWith('/api/updates/');
    const libraryRequest = url.pathname === '/api/library' || url.pathname.startsWith('/api/library/');
    if ((applyingUpdate && !updateRequest) || switchingLibrary) {
      res.setHeader('X-Serigraph-Library', LIBRARY_ID);
      return json(res, 503, { error: 'Serigraph is restarting. No changes were saved; try again after restart.' });
    }
    if (url.pathname.startsWith('/api/')) {
      const operation = !updateRequest && !libraryRequest && req.method !== 'GET';
      if (operation) activeOperations++;
      try { return await handleApi(req, res, url); }
      finally { if (operation) activeOperations--; }
    }
...
// If the launcher disappears, its server and managed children must not linger.
if (process.send) process.on('disconnect', () => shutdown('launcher disconnected'));

async function start(port, attempt = 0) {
  const onError = (e) => {
    // A failed listen leaves its callback attached. Remove it before retry
    // or both callbacks would announce different ports and initialize twice.
    server.off('listening', onListening);
    if (e.code === 'EADDRINUSE' && attempt < 10 && process.env.SERIGRAPH_RESTART !== '1') {
